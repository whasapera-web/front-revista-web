import NavBar from "../../components/NavBar";
export default function Nosotros() {

    return (
        <>
            <main className="bg-violet-900 xl:w-full xl:h-max xl:flex xl:justify-center xl:items-center xl:flex-col">
                <NavBar />
                <section className="xl:w-3/6 xl:h-screen p-2 md:p-16 xl:p-2 flex justify-center items-center flex-col">
                    <img src="/titulo.png" alt="" className="w-72 scale-"/>
                    <p className="text-center p-2">
                        Hola te cuento como comenzó todo esto, viendo la necesidad de trabajar y de la situación de como esta el país decidí hacer una revista que se repartiría enviándola por whatsapp a nuestros clientes, amigos y familiares. A su vez  les pediríamos que ellos también la compartan con sus contactos y que le pidan a estos que también la compartan, de esta manera estaríamos llegando a varios hogares, oficinas y comercios.
                        También decidimos cobrar una publicidad super económica para que se sumen varios comercios y profesionales y estén con nosotros durante todo el año.
                        "Una mano lava la otra", pues bien el 1 de Febrero de 2024 salió la primera Revista cuyo resultado fue excelente y quisimos agregar un Facebook (Whatsappera Revista de Clasificados Nicoleña) y un Instagram (Whatsa_ppera) y bueno ya para la segunda edición se nos volvió a prender la lamparita y creamos una pagina WEB para que cubra otras necesidades aparte de tener todas las publicidades.
                        En dicha pagina encontraran los horarios de Colectivos, las Farmacias de turno, las loterías del día, el horóscopo, las noticias de nuestra ciudad y el enlace directo a FM 97.9 VIDA. y bueno acá estamos cumpliendo metas, queremos alcanzar a llegar a tener nuestra radio Online para también dar a conocer sus negocios.
                        Desde ya muchas gracias por siempre confiar en muestro trabajo.
                        Fernanda Correa
                    </p>
                </section>
            </main>
        </>
    )
}